...
export default function FirstRunPage() {
  ...
  // resolve modo: query > localStorage > "word"
  ...
  async function handleFinish() {
    ...
    setSubmitting(true);
    try {
      // 1) registra tentativa (col: artifacts/{APP_ID}/users/{uid}/data/app/attempts)
      const col = collection(db, "artifacts", APP_ID, "users", uid, "data", "app", "attempts");
      await addDoc(col, {
        mode,
        at: serverTimestamp(),
        payload: {}, // placeholder para respostas
        points: 50,
      });

      // 2) soma +50 no perfil público (regras: o próprio uid pode atualizar)
        ...
        {!done ? (
          <>
            ...
            <div className="mb-5 rounded-lg border bg-slate-50 p-4 text-sm text-slate-600">
              Conteúdo interativo chegará aqui (texto, áudio ou micro-exercício).
              Por enquanto, clique em <strong>{meta.cta}</strong> para registrar sua primeira tentativa.
            </div>
