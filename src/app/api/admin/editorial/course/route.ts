...
type Body = { slug: string; title: string; description?: string };
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await sb.from('courses').insert({ slug, title: body.title.trim(), description: body.description?.trim() || null, created_by: user_id }).select().maybeSingle();
